...
namespace yjc_translate { 
	...
	*/
    ...
    void dfs(int u) {
    	...
    	/*如果为空，跳出当前循环 */
        if (GraphOfAdjacencyList[u].empty()) return;
        ...
        else if (symbol[u] == "assignment" && symbol[GraphOfAdjacencyList[u][0]] == "ID") {//赋值语句
        ...
        else if (symbol[u] == "primary" && symbol[GraphOfAdjacencyList[u][0]] == "NUM") {//规约NUM
        	primary_num_function(tran,u); 
        }
        else if (symbol[u] == "primary" && symbol[GraphOfAdjacencyList[u][0]] == "ID") {    //规约ID
        ...
        else if (symbol[u] == "term" && GraphOfAdjacencyList[u].size() > 1 && symbol[GraphOfAdjacencyList[u][1]] == "*") {//乘法表达式
            term_mul_function(tran,u);
        }
        else if (symbol[u] == "expr" && GraphOfAdjacencyList[u].size() > 1 && symbol[GraphOfAdjacencyList[u][1]] == "+") {//加法表达式
            expr_add_function(tran,u);
        }
        else if (symbol[u] == "rel" && GraphOfAdjacencyList[u].size() > 1) {//关系表达式
            rel_function(tran,u);
        }
        else if (symbol[GraphOfAdjacencyList[u][0]] == "WHILE") {//WHILE语句
            while_function(tran,u);
        }
        else if (symbol[GraphOfAdjacencyList[u][0]] == "IF" && GraphOfAdjacencyList[u].size() == 5) {//IF语句
            if_function(tran,u); 
        }
        else if (symbol[GraphOfAdjacencyList[u][0]] == "IF" && GraphOfAdjacencyList[u].size() == 7) {//IF ELSE语句
    ...
    void main() {
        ...
        for (pair<int, vector<string>> it : attribution[root].code){
		    ...
		    cout << "地址" << it.first << "对应的四元式:" << " (" << it.second[0] << ", " << it.second[1]<< ", " << it.second[2] << ", " << it.second[3] << ")" << endl; 
		} 
